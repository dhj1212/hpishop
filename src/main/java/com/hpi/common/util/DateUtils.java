...
public class DateUtils extends PropertyEditorSupport
{
	...
	*/
	public static String date2Str(Date date, String format) {
	     ...
	     *    修改内容：
	     */
	    public static Date getFormat(Date date,String format) throws AppException
		{
		  try
			{
				if(date==null || date.equals("") || format==null || format.equals("")) return null;
			
				SimpleDateFormat formatter = getSDFormat(format);
	 ...
	 * @date 2022/4/6 17:06
	*/
	public static String getData2String(Date date,String format) throws AppException{
		SimpleDateFormat sdf = new SimpleDateFormat(format);
		return sdf.format(date);
	}
	     ...
	     *    修改内容：
	     */
	    public static long getdiffdateSec(Date begindate, Date enddate) throws AppException {
			try {
				begindate=getFormat(begindate, "yyyy-MM-dd HH:mm:ss");
				enddate=getFormat(enddate, "yyyy-MM-dd HH:mm:ss");
				 long sec = 0;  
				...
				log.error("计算两个日期之间相差的天数时程序出错：(Util类中的getdiffdates方法)"
						+ error.getMessage());
				throw new AppException("计算两个日期之间相差的天数时程序出错：" + error.getMessage()
						+ "重试无效后请联系管理员！");
			}
		}
		 ...
		 *    修改内容：
		 */
		public static long getdiffdateMin(Date begindate, Date enddate) throws AppException {
			try {
				begindate=getFormat(begindate, "yyyy-MM-dd HH:mm:ss");
				enddate=getFormat(enddate, "yyyy-MM-dd HH:mm:ss");
				 long min = 0;  
		 ...
		 *    修改内容：
		 */
		public static long getdiffdateHour(Date begindate, Date enddate) throws AppException {
			try {
				begindate=getFormat(begindate, "yyyy-MM-dd HH:mm:ss");
				enddate=getFormat(enddate, "yyyy-MM-dd HH:mm:ss");
				 long hour = 0;  
		...
		*/
		...
		public static List<String> getBeforeAndAfterMonth(int num) {
			Date now = DateUtils.getDate();
			List<String> list = new ArrayList<>();
			if (num > 0) {
				for (int i = 1; i <= num; i++) {
					Date newDate11 = stepMonth(now, i);
	    ...
	    public static void main(String[] args) {
	    	...
	    	//String title="33updateUPdatefdedelr$efedropddr{rDEL(derr";
	    	//title=title.replace("update", "").replaceAll("del", "").replaceAll("drop", "").replace("insert", "").replaceAll("\\(", "")
			//		.replaceAll("\\)", "").replaceAll("\\{", "").replaceAll("\\}", "").replaceAll("select", "").replaceAll("\\{", "").
			//		replaceAll("UP", "").replaceAll("D", "").replaceAll("SELE", "").replaceAll("\\{", "").replaceAll("\\$", "");
	    	//System.out.println(title);
			//System.out.println(getData2String(date,"yyyy/MM/dd HH:mm:ss"));
			//System.out.println(stepMonth(new Date(),8));
			//System.out.println(getUpMonth(-8));
			//System.out.println(getUpMonth(8));
			/*List<String> list1=getBeforeAndAfterMonth(-8);
			List<String> list2=getBeforeAndAfterMonth(8);
			List<String> list = new ArrayList<>();
			Collections.reverse(list2);
			String moth=DateUtils.date2Str(DateUtils.getDate(),"yyyy-MM");
			list2.add(moth);
			list.addAll(list2);
			list.addAll(list1);
			System.out.println(list);*/
			String s1="90806ffe-4e04-4b24-aad5-08264e41367c";
			String s2="90806ffe-4e04-4b24-aad5-08264e41367c";
			if (s1.equals(s2)){
				System.out.println("title");
			}
